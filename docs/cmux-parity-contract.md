...
| Browser import | partial <!-- cmux-parity:browser-import status=partial --> | Browser panes can import cookies, history, and sessions from Chrome, Firefox, Arc, and other browsers. | `limux browser profiles` discovers local Chrome-family and Firefox profile cookie/history/session store paths, `limux browser profile-data` provides a consent-gated dry-run manifest and raw-store staging copy path for cookies/history/sessions, and `limux browser import-cookies --file <path>` imports JSON or Netscape `cookies.txt` rows into the active WebKit profile through `browser.cookies.import`/WebKitGTK `CookieManager`, including `httpOnly` rows when the running host supports it. The Xvfb smoke harness now covers JSON cookie import response handling against a throwaway local HTTP fixture. SQLite row extraction/decryption, Arc-specific profile discovery, actual history import, and actual session import remain open. | Add SQLite/decryption design for cookie DBs, then history/session ingestion. |
