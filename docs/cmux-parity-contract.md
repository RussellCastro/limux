...
| Notification panel | partial <!-- cmux-parity:notification-panel status=partial --> | Dedicated panel lists pending notifications and can jump to the latest unread item. | Limux exposes toasts, unread workspace state, pane/tab-targeted live notification rows via `notification.list`/`notification.clear`, `notification.jump`/`limux jump-notification`, and a native `Ctrl+Alt+O` notifications panel with jump, clear-one, and clear-all actions. `scripts/xvfb-smoke-test.sh` now covers live notification list, jump/read, and clear flows. | Add native panel shortcut/visual smoke coverage before marking complete. |
...
| Custom commands | partial <!-- cmux-parity:custom-commands status=partial --> | Project-specific commands in `cmux.json` launch from the command palette. | Limux can discover `cmux.json`/`limux.json` project commands via `limux commands` and launch one with `limux run-command <name>` or `limux commands run <name>` through `workspace.create`; the Xvfb smoke harness now verifies discovery and launch with a proof-file command. The native `Ctrl+Shift+P` command palette also lists project commands from the active workspace and launches them as command workspaces. | Add direct native command-palette activation smoke coverage and decide whether command definitions should move to a shared CLI/host crate. |
