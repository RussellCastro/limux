...
| Scriptable browser | partial <!-- cmux-parity:scriptable-browser status=partial --> | Browser panes can be driven through CLI/socket APIs for accessibility tree snapshots, element refs, click/fill, JS eval, and URL control. | The live GTK bridge now supports browser split creation, navigation, URL lookup, title lookup, basic JS eval, DOM/ARIA-derived snapshots, `browser.find.*`, selector or in-memory `@eN` ref click/fill, `browser.get.{text,html,value,attr,count,box,styles}`, and immediate `browser.wait` readiness checks for WebKitGTK browser surfaces. Polling waits, full accessibility-tree parity, frame-stable refs, richer eval serialization, and broader automation remain open. | Add polling waits, type, screenshots, cookies/storage, tab lifecycle, richer smokes, and full accessibility-tree parity. |
