...
| Custom commands | partial <!-- cmux-parity:custom-commands status=partial --> | Project-specific commands in `cmux.json` launch from the command palette. | Limux can discover `cmux.json`/`limux.json` project commands via `limux commands` and launch one with `limux run-command <name>` or `limux commands run <name>` through `workspace.create`. In-app command palette integration remains open. | Wire project commands into the native command palette/settings UI and add live smoke coverage. |
...
| Keyboard shortcuts | partial <!-- cmux-parity:keyboard-shortcuts status=partial --> | cmux provides workspace, surface, split, browser, notification, find, terminal, and window shortcuts with macOS conventions and customization. | Limux maps default behavior to Linux `Ctrl`/`Alt`/`Meta` conventions and documents app/browser/find/notification/terminal/workspace shortcuts, including `Ctrl+W` for focused-tab close, `Ctrl+Alt+W` for focused-pane close, `Ctrl+Alt+J` for latest-notification jump, and `Ctrl+,` for Settings/Keybindings. | Add native command palette and remaining remapping parity decisions. |
