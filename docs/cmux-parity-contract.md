...
| Browser import | partial <!-- cmux-parity:browser-import status=partial --> | Browser panes can import cookies, history, and sessions from Chrome, Firefox, Arc, and other browsers. | `limux browser profiles` discovers local Chrome-family and Firefox profile cookie/history/session store paths, and `limux browser import-cookies --file <path>` imports JSON or Netscape `cookies.txt` rows into the active WebKit profile through `browser.cookies.import`/WebKitGTK `CookieManager`, including `httpOnly` rows when the running host supports it. Chrome-family cookie database extraction/decryption, Arc-specific profile discovery, history import, and session import remain open. | Design an explicit consent-gated profile-data import path before cookie database extraction; then add history/session import. |
