...
| Browser import | partial <!-- cmux-parity:browser-import status=partial --> | Browser panes can import cookies, history, and sessions from Chrome, Firefox, Arc, and other browsers. | `limux browser import-cookies --file <path>` imports JSON or Netscape `cookies.txt` rows into the active WebKit profile through `browser.cookies.import`/WebKitGTK `CookieManager`, including `httpOnly` rows when the running host supports it. Native Chrome/Firefox/Arc profile discovery, history import, and session import remain open. | Add browser-specific profile discovery plus history/session import. |
