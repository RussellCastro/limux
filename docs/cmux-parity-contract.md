...
| Notification panel | partial <!-- cmux-parity:notification-panel status=partial --> | Dedicated panel lists pending notifications and can jump to the latest unread item. | Limux exposes toasts, unread workspace state, pane/tab-targeted live notification rows via `notification.list`/`notification.clear`, `notification.jump`/`limux jump-notification`, and a native `Ctrl+Alt+O` notifications panel with jump, clear-one, and clear-all actions. | Add live smoke coverage before marking complete. |
...
| Custom commands | partial <!-- cmux-parity:custom-commands status=partial --> | Project-specific commands in `cmux.json` launch from the command palette. | Limux can discover `cmux.json`/`limux.json` project commands via `limux commands` and launch one with `limux run-command <name>` or `limux commands run <name>` through `workspace.create`. The native `Ctrl+Shift+P` command palette also lists project commands from the active workspace and launches them as command workspaces. | Add live smoke coverage and decide whether command definitions should move to a shared CLI/host crate. |
