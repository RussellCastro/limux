...
| Scriptable browser | partial <!-- cmux-parity:scriptable-browser status=partial --> | Browser panes can be driven through CLI/socket APIs for accessibility tree snapshots, element refs, click/fill/type/select/check, JS eval, and URL control. | The live GTK bridge now supports browser split creation, navigation, URL lookup, title lookup, basic JS eval, DOM/ARIA-derived snapshots, `browser.find.*`, selector or in-memory `@eN` ref click/fill/type/check/uncheck/select/focus/hover/dblclick/scroll/scroll-into-view/keyboard actions, `browser.get.{text,html,value,attr,count,box,styles}`, and immediate `browser.wait` readiness checks for WebKitGTK browser surfaces. Polling waits, screenshots, cookies/storage, tab lifecycle, full accessibility-tree parity, frame-stable refs, and richer eval serialization remain open. | Add screenshots, polling waits, cookies/storage, tab lifecycle, richer smokes, and full accessibility-tree parity. |
