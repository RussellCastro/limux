...
| Sidebar metadata | partial <!-- cmux-parity:sidebar-metadata status=partial --> | Sidebar shows git branch, linked PR status/number, working directory, listening ports, and latest notification text. | Limux live bridge exposes `sidebar.state` and `limux sidebar-state` prefers it, returning workspace cwd, best-effort git branch, linked PR number/status via `gh pr view`, listening ports tied to workspace processes via `ss` + `/proc/<pid>/cwd`, unread state, and latest sidebar notification text. The metadata is socket/CLI-visible; richer in-sidebar rendering and live smoke coverage remain open. | Add UI refresh/rendering for PR/port fields plus live smoke coverage. |
