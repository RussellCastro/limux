...
| Browser import | partial <!-- cmux-parity:browser-import status=partial --> | Browser panes can import cookies, history, and sessions from Chrome, Firefox, Arc, and other browsers. | `limux browser import-cookies --file <path>` imports JSON or Netscape `cookies.txt` rows into the active WebKit page through `browser.cookies.set`; `httpOnly` cookies are reported as skipped and native Chrome/Firefox/Arc profile, history, and session import remain open. | Add WebKitGTK profile-level cookie/session import and browser-specific profile discovery. |
