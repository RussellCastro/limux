...
| Notification panel | partial <!-- cmux-parity:notification-panel status=partial --> | Dedicated panel lists pending notifications and can jump to the latest unread item. | Limux exposes toasts, unread workspace state, and a live GTK notification model via `notification.list`/`notification.clear` plus `limux list-notifications`/`clear-notifications`. A dedicated in-app panel and jump-to-latest UI remain open. | Add panel UI, jump action, and live smoke coverage. |
