...
| Scriptable browser | partial <!-- cmux-parity:scriptable-browser status=partial --> | Browser panes can be driven through CLI/socket APIs for accessibility tree snapshots, element refs, click/fill, JS eval, and URL control. | The live GTK bridge now supports browser split creation, navigation, URL lookup, and title lookup for WebKitGTK browser surfaces. JS eval, accessibility snapshots, element refs, click/fill, and richer automation remain open. | Add async `browser.eval`, then accessibility snapshot and element-action commands. |
