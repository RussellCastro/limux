...
| Browser split | partial <!-- cmux-parity:browser-split status=partial --> | Browser can open alongside terminals as a split surface. | Limux documents browser splits, the live GTK bridge routes `browser.open_split` to create a browser-only split pane, and the Xvfb smoke harness opens a real browser split. | Add right-neighbor reuse parity before marking complete. |
