...
- Browser command bridge parity beyond the current slices: SQLite/decryption design after the consent-gated profile-data staging path, actual history/session import, full accessibility-tree parity, storage/cookie edge smokes, and full cross-frame/platform refs.
